...
Run:
  CONTEXT_DEV_API_KEY=... OPEN_ROUTER_KEY=... \\
    python -m context_dev.loops.mcp_code_mode "Get stripe.com brand, design tokens, and site scale"
"""
...
from context_dev.mcp.client import (
    extract_domain_from_goal,
    hosted_execute,
    hosted_search_docs,
)
from context_dev.mcp.codegen import build_execute_typescript
from context_dev.mcp.op_map import OP_CREDITS
from context_dev.planning.cli import cmd_mcp

DOC_INDEX = Path(__file__).resolve().parent.parent / "data" / "sdk_doc_index.json"
...
def execute_plan_local(plan: list[dict[str, Any]]) -> tuple[dict[str, Any], list[str], int]:
    ...
    from context_dev.client import (  # lazy: hosted path needs no SDK install
        create_client,
        extract_styleguide,
        retrieve_brand,
        scrape_sitemap,
