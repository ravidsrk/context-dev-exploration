...
- Python: `context_dev/loops/mcp_code_mode.py` → `hosted_search_docs` + `hosted_execute` via `context_dev/mcp/client.py`
- TypeScript: `typescript/src/mcp_code_mode_loop.ts` → same hosted MCP tools

Live evidence: `evidence/runs/agent-loop.log` shows `execute_source: hosted_mcp_execute` and populated `result.identity` / `design_tokens` / `site_scale`.
...
| Artifact | Path |
|----------|------|
| Python agent loop (research scout + LLM plan) | `context_dev/loops/scout.py` |
| MCP Code Mode (search_docs + execute) | `context_dev/loops/mcp_code_mode.py` |
| Python probes | `scripts/run/demos.py` |
| TypeScript probes | `typescript/src/probe.ts` |
| Monid deep runs | `docs/research/MONID_DEEP_RESEARCH.md` |
| Novel build ideas | `docs/agents/AGENT_OPPORTUNITIES.md` |