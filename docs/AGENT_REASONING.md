...
3. **TypeScript is the native execution language** in the sandbox; Python orchestrators should call hosted `execute` (see `agents/mcp_code_mode_loop.py` and `typescript/src/mcp_code_mode_loop.ts`).

This repo runs both paths against the same hosted MCP with live evidence in `evidence/agent-loop.log`.
...
Runnable in **Python** (`agents/mcp_code_mode_loop.py`) and **TypeScript** (`typescript/src/mcp_code_mode_loop.ts`).
...
| `evidence/agent-loop.log` | Live hosted MCP execute proof |
